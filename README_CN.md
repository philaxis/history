...
`mode: number` 会始终以紫色和薄荷色数字显示数量，而不是圆点；点模式在前两行各居中显示 6 个点，再在第三行居中显示超出数量 `+N`。省略时自动选择显示方式。`align` 支持 `left`、`center` 和 `right`。正文日历默认为 `max-width: 502`；`ratio` 是单元格的宽度 ÷ 高度，默认为 `1.2`。`font-size` 在 `0.5–2.0` 之间缩放页眉、星期、日期文字和按钮，默认为 `1.0`。`marker-size` 独立地在 `0.5–2.0` 之间缩放圆点、计数和 `+N`，默认为 `1.2`；宽度缩放仍会同时应用。侧边栏默认为数字标记、单元格比例 `0.6`、字体倍率 `2.0`（最大 `3.0`）和标记倍率 `3.0`（最大 `4.0`），相关设置都集中在 History 设置的最后一组中。设置中可选显示悬停弹窗的 × 按钮，确认后可删除单个历史日期；默认关闭。
